...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                html.H1("🌊 Adopt-a-Drain Dashboard", className="display-4 text-white"),
                html.P("Crystal, Minnesota Drain Adoption Program", className="lead text-white"),
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4("Dashboard Filters", className="card-title"),
                    html.P("Select time period and watershed:", className="text-muted"),
                    dbc.Label("Year"),
                    dcc.Dropdown(
                        id="year-filter",
                    ...
                    dcc.Dropdown(
                        id="watershed-filter",
                        options=[{"label": w, "value": w} for w in watersheds],
                        value="All"
                    ),
                ])
            ], className="shadow-sm")
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                html.H3(id="total-adoptions", className="text-primary mb-0"),
                html.P("Total Adoptions", className="text-muted small")
            ])
        ], className="shadow-sm text-center"), width=3),
        dbc.Col(dbc.Card([
            dbc.CardBody([
                html.H3(id="total-cleanings", className="text-success mb-0"),
                html.P("Total Cleanings", className="text-muted small")
            ])
        ], className="shadow-sm text-center"), width=3),
        dbc.Col(dbc.Card([
            dbc.CardBody([
                html.H3(id="total-collected", className="text-info mb-0"),
                html.P("Total Collected", className="text-muted small")
            ])
        ], className="shadow-sm text-center"), width=3),
        dbc.Col(dbc.Card([
            dbc.CardBody([
                html.H3(id="avg-per-cleaning", className="text-warning mb-0"),
                html.P("Average per Cleaning", className="text-muted small")
            ])
        ], className="shadow-sm text-center"), width=3),
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="monthly-cleanings")
            ])
        ], className="shadow-sm"), width=6),
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="monthly-adoptions")
            ])
        ], className="shadow-sm"), width=6),
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="monthly-collected")
            ])
        ], className="shadow-sm"), width=6),
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="adoption-cleaning-trends")
            ])
        ], className="shadow-sm"), width=6),
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="debris-distribution")
            ])
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="cleaning-locations-map")
            ])
        ], className="shadow-sm"), width=12),
    ], className="mb-4"),
    ...
    dbc.Row([
        dbc.Col(dbc.Card([
            dbc.CardBody([
                dcc.Graph(id="watershed-analysis")
            ])
        ], className="shadow-sm"), width=12),
    ], className="mb-4"),
    ...
    html.Footer([
        ...
        html.P(
            ["Data sourced from the ", 
             html.A("Adopt-a-Drain", href="https://mn.adopt-a-drain.org/", className="text-primary"),
             " program for Crystal, Minnesota. Dashboard created with ",
             html.A("Dash", href="https://dash.plotly.com/", className="text-primary"),
             "."],
...
def update_metrics(selected_year, selected_watershed):
    ...
    if selected_year != "All":
        filtered_adoptions = filtered_adoptions[filtered_adoptions["Adoption Date"].dt.year == selected_year]
        filtered_cleanings = filtered_cleanings[filtered_cleanings["Cleaning Date"].dt.year == selected_year]
    
    if selected_watershed != "All":
        filtered_adoptions = filtered_adoptions[filtered_adoptions["Watershed"] == selected_watershed]
        filtered_cleanings = filtered_cleanings[filtered_cleanings["Watershed"] == selected_watershed]
...
def update_monthly_cleanings(selected_year, selected_watershed):
    ...
    monthly_cleanings = filtered_cleanings.set_index("Cleaning Date").resample("ME").size()
    fig = px.line(
        monthly_cleanings,
...
def update_monthly_adoptions(selected_year, selected_watershed):
    ...
    monthly_adoptions = filtered_adoptions.set_index("Adoption Date").resample("ME").size()
    fig = px.line(
        monthly_adoptions,
...
def update_monthly_collected(selected_year, selected_watershed):
    ...
    monthly_collected = filtered_cleanings.set_index("Cleaning Date")["Collected Amount"].resample("ME").sum()
    fig = px.line(
        monthly_collected,
...
def update_yearly_summary(selected_year, selected_watershed):
    ...
    yearly_summary = pd.DataFrame({
        "Year": all_years,
        "Adoptions": filtered_adoptions["Adoption Date"].dt.year.value_counts().reindex(all_years, fill_value=0),
        "Cleanings": filtered_cleanings["Cleaning Date"].dt.year.value_counts().reindex(all_years, fill_value=0)
    })
...
def update_yearly_trends(selected_year, selected_watershed):
    ...
    yearly_adoptions = filtered_adoptions.set_index("Adoption Date").resample("YE").size()
    yearly_cleanings = filtered_cleanings.set_index("Cleaning Date").resample("YE").size()
...
def update_watershed_analysis(selected_year, selected_watershed):
    ...
    watershed_stats = filtered_cleanings.groupby("Watershed").agg({
        "ID": "count",
        "Collected Amount": "sum"
    }).round(1).reset_index()
    
    watershed_stats.columns = ["Watershed", "Number of Cleanings", "Total Debris Collected (lbs)"]
